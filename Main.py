...
class estruturaDados:
    ...
    def tendencia(DadosTestando, curva):
        ...
        pos = []
        if inclinacao == 1:
            saida.append("Tendência de Alta")
        if (inclinacao == -1):
            saida.append("Tendência de Baixa")
        if (inclinacao == 0):
            saida.append("Tendência Lateral")
        pos.append(1)
        cont = 0
        acumulado = 0
        index = 1
    ...
    def segmentaCluster(Z, K, plot):
        ...
        print("Quantidade Dias: ", len(results))
        # check the results
        s = pd.Series(results)
        clusters = s.unique()
        i = 0
        for c in clusters:
            ...
            if (len(cluster_indices) >= 4) & (plot):
                print("Cluster %d possui %d Semanas: Indice[%d]" % (c, len(cluster_indices), i))
            i = i + 1
    ...
    def printClusters(listaCluster, Soma, Representacao, listaOrdenada, DadosTreinamento):
        ...
        for i in range(0, len(listaCluster)):
            ...
            if (Acumulado <= 80) & (len(lista) > 4):
                print('Cluster %d' % (i))
                print('Possui %d dias' % len(lista))
                Acumulado = Percentual + Acumulado
                print('Representação: %.2f' % Percentual)
                print('Representação Acumulada: %.2f' % Acumulado)
    ...
    def SimulacaoEstrategia(dia, plot, stoplossAux, DadosEntrada, listaMedias, Representacao, parametrosNegociacao, Dados):
        ...
        VendidoHoraVenda = 0
        if inicio == 0:
            print("Dia Com Problema nos Dados", dia)
        else:
                    ...
                    if (i < ultimoNegocio) & (controleCompra >= 0):
                        ...
                        if Negocio == -1:
                            if compras > 0:
                                ...
                                if (plot):
                                    print('Venda: %.2f' % DadosEntrada.iloc[dia][i])
                            ...
                            if vendas == 0:
                                if DadosEntrada.iloc[dia][i] != 0:
                                    ...
                                    if (plot):
                                        ...
                                        print('Venda Descoberta: %.2f' % valorVenda)
